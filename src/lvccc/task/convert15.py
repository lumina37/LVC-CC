...
from ..config import CalibCfg, RLC15Cfg, get_config
...
@dcs.dataclass
class Convert15Task(ConvertTask, NonRootTask["Convert15Task"]):
    """
    Multi-view conversion (with RLC-1.5, PNG I/O).
    """

    task: ClassVar[str] = "convert15"
    ...
    def calib_cfg_path(self) -> Path:
        return Path("config") / self.seq_name / "rlc15" / "calib.xml"

    @functools.cached_property
    def param_cfg_path(self) -> Path:
        return Path("config") / self.seq_name / "rlc15" / "param.cfg"
    ...
    def run(self) -> None:
        ...
        rlccfg = RLC15Cfg.from_file(self.param_cfg_path)
        ...
        # Convert
        convert_cmds = [
            config.app.convertor.RLC15,
            rlccfg_dstpath,
        ]

        run_cmds(convert_cmds, cwd=img_dstdir)
