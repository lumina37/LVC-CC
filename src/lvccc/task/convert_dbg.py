...
@dcs.dataclass
class ConvertDbgTask(NonRootTask["ConvertDbgTask"]):
    """
    Multi-view conversion (with TLCT). For debugging purpose.
    """
    ...
    def run(self) -> None:
        ...
        copy_task = CopyTask(seq_name=self.seq_name, frames=self.frames)
        rawyuv_srcpath = get_any_file(copy_task.dstdir, "*.yuv")
        yuv_srcpath = get_any_file(self.srcdir, "*.yuv")
        yuv_dstdir = self.dstdir / "yuv"
        mkdir(yuv_dstdir)

        psize_bin = Path(config.app.convertor.TLCT).with_name("tlct-patchsize")
        mv_bin = Path(config.app.convertor.TLCT).with_name("tlct-multiview")
        ...
        for yuv_path in list(yuv_dstdir.glob("*.yuv")):
